...
class MyApp(QtGui.QWidget):
    ...
    def test(self):
        ...
        self.threadPool.append(
            GenericThread(self.addBatch2, "from generic thread using signal ",
                          delay=0.3))
...
class GenericThread(QtCore.QThread):
    ...
    def run(self):
        ...
        process = Popen([ 'C:/Users/sped/Documents/GitHub/threadingTest/test.py', '-d'], stdout=PIPE,
                        stderr=PIPE, shell=True)
        stdout, stderr = process.communicate()
        self.emit(QtCore.SIGNAL('update(QString)'),
                  "from work thread " + str(stdout))
        #print stdout,stderr
        return
